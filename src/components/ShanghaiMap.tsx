...
const ShanghaiMap: React.FC<ShanghaiMapProps> = ({
  ...
  const handleDistrictClick = useCallback((districtId: string) => {
    ...
    if (chartRef.current) {
      ...
      const targetDistrict = shanghaiDistricts.find(d => d.id === districtNames[districtId]);
      if (targetDistrict) {
        onDistrictSelect(targetDistrict);
  ...
  const projectsInSelectedDistrict = useMemo(() => {
    ...
    return filtered;
  }, [selectedDistrict, priceFilter, areaFilter, statusFilter, sortBy]);
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-gradient-to-br from-gray-900 via-black to-gray-900">
      ...
      {showSubDistrictCard && selectedDistrict && (
        <div className="absolute inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gray-900 rounded-2xl border border-orange-500/30 shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto animate-in fade-in duration-300">
            <div className="p-6 border-b border-white/10">
              <div className="flex items-center justify-between mb-4">
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {selectedDistrict.subDistricts.map((subDistrict) => (
                  <div
                    ...
                    className="p-4 rounded-lg border border-white/10 hover:border-orange-500/50 hover:bg-orange-500/5 transition-all cursor-pointer group"
                  >
                    ...
                    <p className="text-sm text-gray-400 mb-3">{subDistrict.description}</p>
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-gray-500">
                        楼盘数量: {newHomeProjects.filter(p => p.districtId === selectedDistrict.id).length}
                      </span>
                      <ChevronRight className="w-4 h-4 text-orange-500/50 group-hover:text-orange-400 transition-all" />
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-8 border-t border-white/10 pt-6">
                <h4 className="text-lg font-medium text-white mb-4">热门楼盘推荐</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {projectsInSelectedDistrict.slice(0, 4).map((project) => (
                    <div
                      key={project.id}
                      onClick={() => handleProjectClick(project)}
                      className="p-4 rounded-lg border border-white/10 hover:border-orange-500/50 hover:bg-white/5 transition-all cursor-pointer group"
                    >
                      <div className="flex items-start gap-3">
                        ...
                        />
                        <div className="flex-1 min-w-0">
                          <h5 className="text-sm font-medium text-white truncate mb-1 group-hover:text-orange-400 transition-colors">
                            {project.name}
                          </h5>
                          <p className="text-xs text-gray-400 truncate mb-2">{project.subDistrictId}</p>
                          <div className="flex items-center justify-between">
